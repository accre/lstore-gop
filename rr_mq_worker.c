...
void build_send_increment(mq_portal_t *portal)
{
    ...
    if(status != 0)
        log_printf(0, "WORKER: Failed sending INCREMENT request to server - error = %d\n", status);
    else
...
void build_send_register(mq_context_t *mqc, mq_portal_t *wp, mq_command_table_t *tbl)
{
    ...
    if(status != OP_STATE_SUCCESS) {
        log_printf(0, "WORKER: Failed sending REGISTER request to server - error = %d\n", status);
    } else {
...
void build_send_deregister(mq_context_t *mqc, mq_portal_t *wp)
{
    ...
    if(status != OP_STATE_SUCCESS)
        log_printf(0, "WORKER: Failed sending DEREGISTER request to server - error = %d\n", status);
    else
